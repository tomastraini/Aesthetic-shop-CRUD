...
like '%" & DataGridView1.Rows(e.RowIndex).Cells(e.ColumnIndex).Value & "%'")
            ...
            Try
                ...
                Dim da As New SqlDataAdapter(com)
                da.Fill(ds, "clientes")
                Cargar_consulta.TextBox2.Text = ds.Tables("clientes").Rows(0).Item(0)
                Cargar_consulta.TextBox5.Text = ds.Tables("clientes").Rows(0).Item(1)
                Cargar_consulta.TextBox5.Text = Cargar_consulta.TextBox5.Text + " " + ds.Tables("clientes").Rows(0).Item(2)
                Close()
            ...
            Dim sql2 As String = ("select max(cast(id_consulta as int)) from consulta")
            Dim com2 As New SqlCommand(sql2, con2)
            Try
                Dim ds2 As New DataSet
                Dim da2 As New SqlDataAdapter(com2)
                da2.Fill(ds2, "clientes")
                Cargar_consulta.TextBox1.Text = ds2.Tables("clientes").Rows(0).Item(0)
                Cargar_consulta.TextBox1.Text = Cargar_consulta.TextBox1.Text + 1
                Close()
                Cargar_consulta.Show()
            Catch ex As Exception
                MsgBox(ex.Message)
